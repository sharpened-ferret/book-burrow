    ...
    <div class="py-12">
        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mt-4">
            <div class="p-6 text-gray-900 dark:text-gray-100">
                <form method="POST" action="{{ route('posts.update', $post->id) }}" class="bg-white dark:bg-gray-800 rounded px-8 pt-6 mb-4">
                    @method('PUT')
                    @csrf
                    <h1 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight pb-8">{{ __('Edit Post for book: ') }}{{$post->book->title}}</h1>
                    ...
                    <x-secondary-button onclick="location.href='{{ route('posts.show', $post->id) }}'">{{ __('Cancel') }}</x-primary-button>
